...
# ── Cryptic clues — loaded from db/cryptic_clues.json ────────────────────────
...
if File.exist?(cryptic_path)
  ...
  cryptic_clues.each do |c|
    ...
    Riddle.find_or_create_by!(question: c["clue"]) do |r|
      r.assign_attributes(
  ...
  puts "Loaded #{cryptic_clues.size} cryptic clues from #{cryptic_path}"
else
